...
var	tss_auto = tss_objects.tss_auto,
	...
	tss_time = tss_objects.tss_time,
	tss_images = tss_objects.tss_images,
	tss_mob = tss_objects.tss_mob;

jQuery.noConflict();

if (!/android|iphone|ipod|series60|symbian|windows ce|blackberry/i.test(navigator.userAgent) || tss_mob) {
	jQuery(function ($) {
		"use strict";
		...
		// slimbox for image maps
		if (tss_maps) {
			$("area").slimbox({slideInterval: tss_time, scaler: tss_scaling});
		}

		// standard slimbox on 'rel' links
		$("a[rel^='lightbox']").slimbox({slideInterval: tss_time, scaler: tss_scaling}, null, function (el) {
			return (this === el) || ((this.rel.length > 8) && (this.rel === el.rel));
		});

		// slideshow slimbox on hrefs starting with '#s'
		$("a[href^='#slide']").click(function () {
			slideshow();
		});

		// look in the url for '#s' as well
